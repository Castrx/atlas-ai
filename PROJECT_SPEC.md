...
Status: **Fases 0–3 concluídas; hardening de segurança da Fase 6 concluído (escopo reduzido).** Backend (Express/TypeScript), frontend (React), Structured Outputs, Tool Calling e integração real com o Atlas ERP estão implementados e funcionais, com CI (GitHub Actions) rodando build e testes automatizados do backend e build do frontend a cada push/PR. O provedor de LLM em uso é a OpenRouter (`google/gemma-4-26b-a4b-it:free`), API compatível com o padrão OpenAI — ver ADR-024. Rate limiting, CORS explícito e headers HTTP de segurança (reservados pelo ADR-009) foram implementados sem dependência nova — ver ADR-025. Fases 4 (MCP) e 5 (RAG) ainda não foram iniciadas; autenticação/RBAC do próprio Atlas AI seguem fora de escopo (ver ADR-025).
...
### Fase 6 — Segurança (hardening de rede concluído; escopo reduzido)
...
> Os limites estruturais permanentes da Seção 10 (nunca acesso direto a banco, tools explícitas e validadas, segredos só em env vars) já estão em vigor desde a Fase 1. Validação de tool calls (Zod, ADR-020) e defesa estrutural contra prompt injection indireta (mensagens `role: "tool"` sempre tratadas como dado, ADR-011/ADR-017) também já existiam desde a Fase 3. O hardening de rede que o ADR-009 havia reservado para esta fase — rate limiting, CORS explícito, headers HTTP de segurança — foi implementado (ver ADR-025), sem dependência nova (`helmet`/`express-rate-limit` avaliados e descartados). Autenticação, autorização e RBAC do próprio Atlas AI (isto é, do endpoint `POST /api/chat` em si, não da integração com o Atlas ERP, que já os tem via ADR-022) foram avaliados e deliberadamente deixados fora de escopo — ver ADR-025.
...
### ADR-022 — `HttpAtlasErpClient`: conta de serviço `USER` autenticada por JWT, `fetch` nativo, sem retry de permissão

**Decisão:** A integração real com o Atlas ERP (Fase 3b) autentica-se como uma conta de serviço **role `USER`**, criada previamente via `POST /auth/register` do próprio Atlas ERP (que sempre cria `USER`, nunca `ADMIN`, independentemente do que for enviado em `role` — comportamento já existente no Atlas ERP, não alterado por este projeto). `HttpAtlasErpClient` implementa `AtlasErpClient` usando somente `fetch` nativo do Node — nenhuma dependência HTTP nova (axios, node-fetch). A autenticação vive em um módulo próprio, `atlas-erp-auth.ts` (`createAtlasErpAuth`): faz `POST /auth/login`, guarda o token JWT em memória do processo (nunca logado, nunca persistido, nunca devolvido à LLM), e nunca decodifica nem confia nos claims do token — só o repassa como `Authorization: Bearer <token>`. Em um `401` (token expirado/inválido), o cliente relogar e repete a chamada **uma única vez**; em um `403` (autenticado mas sem permissão), **nunca** tenta relogar — um token novo não resolveria um problema de permissão, e insistir mascararia um erro de configuração real. As credenciais da conta de serviço entram só por variável de ambiente (`ATLAS_ERP_BASE_URL`, `ATLAS_ERP_SERVICE_EMAIL`, `ATLAS_ERP_SERVICE_PASSWORD`), sem default, fail-fast (mesmo padrão do `OPENAI_API_KEY`, ver ADR-005).
**Contexto:** Os 4 controllers do Atlas ERP relevantes às 6 tools (`ProductController`, `CustomerController`, `SaleController`, `DashboardController`) exigem só `hasAnyAuthority('USER','ADMIN')` nos endpoints `GET` — `USER` já basta para leitura completa. O Atlas ERP também expõe um mecanismo de bootstrap de `ADMIN` (`ADMIN_BOOTSTRAP_EMAIL`/`ADMIN_BOOTSTRAP_PASSWORD`), avaliado e descartado para este uso.
**Alternativas consideradas:** conta `ADMIN` via bootstrap (rejeitada — privilégio maior que o necessário, viola o Princípio 2 do PROJECT_SPEC.md, "menor privilégio", já que nenhuma das 6 tools é de escrita); `axios`/`node-fetch` (rejeitados — `fetch` nativo do Node já cobre tudo que é preciso aqui, sem dependência nova, Princípio 7); relogar também em `403` "por garantia" (rejeitado — um token novo da mesma conta não muda o papel/permissão dela; retry cego esconderia um erro real de configuração, ex.: conta de serviço criada com o papel errado).
**Justificativa:** `USER` é o menor privilégio que ainda cumpre o objetivo (leitura completa das 4 áreas de dado). Manter a autenticação isolada em `atlas-erp-auth.ts` (único módulo que conhece e-mail/senha da conta de serviço) segue o mesmo padrão já usado para `OPENAI_API_KEY`/`OpenAiProvider` (ADR-016) — só um módulo por segredo. Retry único em `401` cobre o caso normal (expiração de token por `JWT_EXPIRATION`) sem risco de loop; a distinção 401-relogar/403-não-relogar espelha exatamente a semântica que o próprio Atlas ERP já documenta na diferença entre `BadCredentialsException`/`JwtAuthenticationFilter` (401) e `AccessDeniedException` (403).

### ADR-023 — Adaptação de contrato: `HttpAtlasErpClient` nunca inventa dado que o Atlas ERP não fornece

**Decisão:** Onde a API real do Atlas ERP não corresponde exatamente ao que uma tool promete, o contrato é adaptado para refletir a API real, nunca preenchido com dado inventado — cada limitação abaixo é deliberada e documentada em código (`http-atlas-erp-client.ts`, `atlas-erp-client.ts`):
- **`search`/`onlyActive` (`getProducts`, `getCustomers`) e `customerId`/`status`/período (`getSales`)**: `GET /products`, `GET /customers` e `GET /sales` não aceitam nenhum desses parâmetros no Atlas ERP — `HttpAtlasErpClient` busca a lista completa (nenhum dos três endpoints pagina) e filtra client-side, com os mesmos critérios que a tool já promete à LLM.
- **`getLowStockProducts`**: não existe endpoint dedicado no Atlas ERP — só um recorte truncado (top 5) dentro de `GET /dashboard`. `HttpAtlasErpClient` reaproveita `GET /products` (mesma chamada de `getProducts`) e filtra client-side por `active && stock <= minimumStock` — o mesmo critério da query real do Atlas ERP (`ProductRepository.countLowStockProducts`), só que sem o limite de 5 itens. Considerado seguro para o volume atual (catálogo pequeno, nenhum dos dois lados pagina); se o catálogo crescer, isto precisa ser revisto.
- **`getSalesSummary({startDate, endDate})`**: não existe endpoint de resumo com período arbitrário — `GET /dashboard` só dá hoje/mês corrente/últimos 7 dias fixos. `HttpAtlasErpClient` calcula `totalSales`/`totalRevenue`/`averageTicket` agregando os dados reais de `GET /sales` filtrados client-side por período — nunca um valor fictício, mas nenhum cálculo feito no Atlas ERP.
- **`status` de venda (`AtlasSale.status`)**: o DTO real de venda (`SaleResponse`) não expõe `status`, mesmo a entidade tendo o campo. `AtlasSale.status` passou a ser `AtlasSaleStatus | null` (era obrigatório): em `getSales`, todo resultado vem de `GET /sales`, que o próprio Atlas ERP já filtra para `ACTIVE` no servidor (`SaleService.findAll` → `saleRepository.findByStatus(ACTIVE)`) — `"ACTIVE"` aqui é uma dedução provada pelo endpoint, não um valor inventado; filtrar por `"CANCELLED"` devolve legitimamente uma lista vazia (limite real, não bug). Em `getSaleById`, `GET /sales/{id}` **não filtra por status** e o DTO não informa qual é — o status é sempre `null` ("desconhecido"), nunca um valor chutado.
- **"Não encontrado" em `getSaleById` é HTTP 409, não 404**: `SaleService.findById` lança `BusinessException` (não `ResourceNotFoundException`) quando a venda não existe, e o `GlobalExceptionHandler` do Atlas ERP mapeia `BusinessException` para 409 Conflict. Como esse é o único motivo de falha de negócio possível numa leitura por id, `HttpAtlasErpClient` trata 409 **somente na rota `GET /sales/{id}`** como "não encontrado" (`null`, que `ToolRegistry` já trata como `empty`) — em qualquer outra rota, 409 continua sendo um erro genérico.
...
### ADR-025 — Hardening de rede da Fase 6: rate limiting, CORS e headers HTTP sem dependência nova; autenticação do próprio Atlas AI fora de escopo

**Decisão:** Implementados, todos sem dependência nova: (1) rate limiting em memória, por IP, janela fixa (`src/middleware/rate-limit.ts`), aplicado somente em `/api` (o único endpoint que aciona LLM/Atlas ERP), com limite padrão de 30 requisições/60s, configurável via `RATE_LIMIT_WINDOW_MS`/`RATE_LIMIT_MAX_REQUESTS`; excedido, responde HTTP 429 (`RateLimitError`) pelo mesmo pipeline de erro do ADR-007; (2) CORS explícito (`src/middleware/cors.ts`) via allowlist de uma única origem, opcional (`CORS_ALLOWED_ORIGIN`) — sem configurar, nenhum header de CORS é enviado, preservando o comportamento restritivo que a aplicação já tinha; nunca `*`, nunca junto de credentials; (3) headers HTTP de segurança escritos à mão (`src/middleware/security-headers.ts`): `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Cross-Origin-Resource-Policy`, `Content-Security-Policy: default-src 'none'` (justificado por a API só servir JSON, nunca HTML/assets) e `X-Powered-By` desligado via `app.disable("x-powered-by")`. Autenticação/autorização/RBAC do próprio endpoint `POST /api/chat` (distinto da autenticação Atlas AI → Atlas ERP, já coberta pelo ADR-022) foram avaliadas e deliberadamente deixadas fora de escopo.
**Contexto:** ADR-009 já reservava rate limiting, CORS e headers HTTP para quando houvesse "exposição real (frontend, deploy)" — o frontend React já existe desde a consolidação da Fase 3. Esta revisão fecha esse hardening pendente, sem alterar arquitetura, tool calling, integração com o Atlas ERP ou provedor/modelo de LLM (fora de escopo explícito desta rodada).
**Alternativas consideradas:** adotar `helmet` para os headers e `express-rate-limit` para o rate limiting — ambos são exatamente o par que o próprio ADR-009 citou como exemplo, avaliados e descartados aqui: a necessidade real (um pequeno conjunto fixo de headers estáticos; um limitador em memória, single-process, sem necessidade de armazenamento compartilhado) é pequena o bastante para não justificar duas dependências novas, seguindo o mesmo princípio já aplicado ao logger próprio no ADR-008 (Princípio 7 do PROJECT_SPEC.md); implementar autenticação (API key, JWT próprio, etc.) no `POST /api/chat` (rejeitado — o Atlas AI, como está, é um assistente single-tenant para demonstração/portfólio, sem conceito de usuário/sessão próprio; adicionar autenticação sem um modelo de usuário real seria infraestrutura sem necessidade comprovada, e contrariaria a instrução explícita desta rodada de não criar funcionalidade nova); rate limiting com armazenamento compartilhado (Redis) para suportar múltiplas instâncias (rejeitado — o projeto roda como processo único, sem deploy multi-instância definido; adicionar Redis agora seria infraestrutura complexa sem necessidade real, o que esta rodada também instruiu explicitamente a não fazer).
**Justificativa:** Os três itens implementados fecham exatamente a lacuna que o ADR-009 já havia identificado e nomeado, com a menor superfície de código nova possível — middlewares pequenos, sem estado compartilhado externo, testados (`tests/unit/rate-limit.test.ts`, `tests/unit/cors.test.ts`, `tests/unit/security-headers.test.ts`, `tests/integration/security.test.ts`) e reversíveis (trocar por `helmet`/`express-rate-limit` depois, se a escala justificar, é uma troca isolada de um arquivo cada, não uma reescrita). Deixar autenticação do próprio Atlas AI fora de escopo é consistente com o Princípio 8 do PROJECT_SPEC.md ("não implementar funcionalidades apenas para 'engordar' o currículo") — o projeto não tem hoje um requisito real de múltiplos usuários/tenants que a justifique.
